...
class FormStore {
        ...
        case 'password':
          if (rule.password) {
            ...
            error.message = error.hasError
              ? rule.message ?? `Password must be 8 char long with 1 number and 1 capital and small alphabet`
              : '';
          }
          break;
        case 'pattern':
  ...
  transformField = (_, __) => {
    ...
    const formattedValue = (___, type) => {
      ...
      if (type === 'postal_code') {
        const postal_code = ___.split('').slice(0, 6);
        // even must be numbers and odd must be apphabats
        const isValid = postal_code.every((v, i) => ((i + 1) % 2 === 0 ? /^[0-9]$/.test(v) : /^[a-zA-Z]$/.test(v)));
        if (isValid) {
          return postal_code.join('');
        }
        // else we remove the faulty characters
        return postal_code.filter((v, i) => ((i + 1) % 2 === 0 ? /^[0-9]$/.test(v) : /^[a-zA-Z]$/.test(v))).join('');
      }
      return ___;
    };
    return formattedValue(value, regex.changeRegex);
  };
  ...
  setFieldsValue = (curStore, validate = false) => {
    ...
    if (curStore) {
      Object.entries(curStore).forEach(([key, value]) => {
        ...
        if (!validate) this.validateField(key, curStore[key]);
      });
